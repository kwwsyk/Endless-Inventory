...
import com.kwwsyk.endinv.neoforge.data.EndlessInventoryData;
...
public final class ServerLevelEndInv {
    ...
    public static boolean hasEndInvUuid(Player player){
        if(player.hasData(ModInitializer.ENDINV_UUID)){
            if(player.getData(ModInitializer.ENDINV_UUID)== ModInitializer.DEFAULT_UUID){
    ...
    private static EndlessInventory getPlayerDefaultEndInv(Player player){
        return levelEndInvData.fromUUID(player.getData(ModInitializer.ENDINV_UUID));
    }
}
